...
static void ShowAdapterInfo()
{
    ...
    if (dwRetVal = GetAdaptersInfo(pAdaptorInfo, &ulOutBufLen)) {
        ...
        while (pAdaptorInfo) {
            ...
            switch (pAdaptorInfo->Type)
            {
            case MIB_IF_TYPE_OTHER:
                cout << "\tNetwork Adapter Type : " << "OTHER" << endl;
                break;
            case MIB_IF_TYPE_ETHERNET:
                cout << "\tNetwork Adapter Type : " << "ETHERNET" << endl;
                break;
            case MIB_IF_TYPE_TOKENRING:
                cout << "\tNetwork Adapter Type : " << "TOKENRING" << endl;
                break;
            ...
            case MIB_IF_TYPE_LOOPBACK:
                cout << "\tNetwork Adapter Type : " << "LOOPBACK" << endl;
                break;
            case MIB_IF_TYPE_SLIP:
                cout << "\tNetwork Adapter Type : " << "SLIP" << endl;
                break;
            default:
            ...
            cout << "\tMAC Address\t: ";
            for (DWORD i = 0; i < pAdaptorInfo->AddressLength; i++)
                if (i < pAdaptorInfo->AddressLength - 1)
                {
                    printf("%02X-", pAdaptorInfo->Address[i]);
                }
                else
                {
                    printf("%02X\n", pAdaptorInfo->Address[i]);
                }
            cout << "\tIP Address List\t:" << endl;
            //可能网卡有多IP,因此通过循环去判断
            IP_ADDR_STRING *pIpAddrString = &(pAdaptorInfo->IpAddressList);
            int IPnumPerNetCard = 0;
            do
            {
                cout << "\t\tNo.\t\t: " << ++IPnumPerNetCard << endl;
                cout << "\t\tIP Address\t: " << pIpAddrString->IpAddress.String << endl;
                cout << "\t\tIP Mask\t\t: " << pIpAddrString->IpMask.String << endl;
                cout << "\t\tGateway\t\t: " << pAdaptorInfo->GatewayList.IpAddress.String << endl;
